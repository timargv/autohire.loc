@foreach ($messages->orderBy('id')->with('user')->get() as $message)
    <div class="card mb-3
        ...
        <div class="card-header border-0 text-right text-muted small">
            {{ $message->created_at->diffForHumans() }} <span class="pl-3">@if(!$message->isMessageOwn() && ($message->user->isAdmin() || $message->user->isModerator())) Отдел технической поддержки @else {{ $message->user->name }}  @endif</span>
        </div>
    </div>
@endforeach
