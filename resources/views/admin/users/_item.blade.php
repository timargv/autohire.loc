    ...
    @foreach ($users as $user)
        <tr>
            ...
            <td>
                ...
                @endif
                @if ($user->isActive())
                    <span class="badge bg-light-blue">{{ $user->statusList()[$user->status] }}</span>
                @endif
            </td>
            <td>
                @if ($user->isAdmin())
                    <span class="badge bg-danger font-weight-light">{{ $user->rolesList()[$user->role] }}</span>
                @elseif ($user->isModerator())
                    <span class="badge bg-success font-weight-light">{{ $user->rolesList()[$user->role] }}</span>
                @else
                    <span class="badge bg-secondary font-weight-light">{{ $user->rolesList()[$user->role] }}</span>
                @endif

                @if($groups = $user->groups)
                    @foreach($groups as $group)
                        <div><span class="badge bg-info text-white font-weight-light small">{{ $group->name }}</span></div>
                    @endforeach
                @endif
            </td>
        </tr>
